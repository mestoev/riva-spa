...
const NAV = [
  { href: "/master", label: "Сегодня", exact: true },
  { href: "/master/bookings", label: "Все записи" },
  { href: "/master/schedule", label: "Расписание" },
...
export function MasterMobileNav({
  ...
  const currentLabel = NAV.find((n) => isActive(n.href, n.exact))?.label ?? "Меню";

  return (
    <>
      <header className="lg:hidden bg-ink text-bg-0 px-4 py-3 sticky top-0 z-40 flex items-center justify-between gap-3">
        <Link href="/master" className="flex items-center gap-2 min-w-0">
        ...
        >
          ...
          <div className="absolute top-0 left-0 right-0 bg-ink text-bg-0 p-5 pb-7 max-h-[90vh] overflow-y-auto shadow-lg">
            <div className="flex items-center justify-between mb-4">
              <span className="serif text-[20px] font-light tracking-wider">RIVA</span>
              <button
                ref={closeBtn}
                type="button"
                onClick={() => setOpen(false)}
            ...
            <div className="bg-bg-0/10 rounded-lg p-3 mb-5">
              <div className="text-[11px] text-bg-0/60">Вы вошли как</div>
              <div className="font-medium mt-0.5">{masterName}</div>
              <div className="text-[12px] text-bg-0/60 mt-0.5">{masterRole}</div>
            </div>

            <nav className="flex flex-col gap-1" aria-label="Меню кабинета мастера">
              {NAV.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center justify-between px-4 py-3.5 rounded-md text-[15px] ${
                      active ? "bg-bg-0/15 text-bg-0" : "text-bg-0/80 hover:bg-bg-0/10"
                    }`}
                  >
